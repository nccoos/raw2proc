...
Spongenet mini_andi pressure parameters sponge data.
"""
...
def parser(platform_info, sensor_info, lines):
    ...
    data = {
        ...
        'pressure'     : n.array(n.ones((num_samples,)) * n.nan, dtype=float),
    ...
    for (sample_index, sample) in enumerate(_data.devices):
        ...
        try:
            data['pressure'][sample_index] = float(sample.sensors
                                                   [sensor_info["id_number"]].
                                                   points[sensor_info
...
def creator(platform_info, sensor_info, data):
    ...
    var_atts = {
        ...
        'pressure' : {'short_name' : 'pressure',
                      'long_name': sensor_info['pressure_description'],
                      'standard_name': 'water_pressure',
                      'units': 'none',
                      'id_number' : sensor_info['pressure_id'],
                      'type' : sensor_info['pressure_type'],
                      'format' : sensor_info['pressure_format'],
                      'non_standard_units' : sensor_info['pressure_units'],
                      'range_min' : sensor_info['pressure_range_min'],
                      'range_max' : sensor_info['pressure_range_max'],
                         },
