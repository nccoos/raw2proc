...
Spongenet mini_andi system parameters sponge data.
"""
...
def parser(platform_info, sensor_info, lines):
    ...
    data = {
        ...
        'voltage'  : n.array(n.ones((num_samples,)) * n.nan, dtype=float),
        'memory'   : n.array(n.ones((num_samples,)) * n.nan, dtype=int),
        'interval' : n.array(n.ones((num_samples,)) * n.nan, dtype=int),
           }
    ...
    for (sample_index, sample) in enumerate(_data.devices):
        ...
        try:
            data['voltage'][sample_index] = float(sample.sensors
                                                  [sensor_info["id_number"]].
                                                  points[sensor_info
                                                         ["voltage_description"]]
                                                  ["value"])
        ...
        try:
            data['memory'][sample_index] = int(sample.sensors
                                               [sensor_info["id_number"]].
                                               points[sensor_info
                                                      ["memory_description"]]
                                               ["value"])
        ...
        try:
            data['interval'][sample_index] = int(sample.sensors
                                                 [sensor_info["id_number"]].
                                                 points[sensor_info
...
def creator(platform_info, sensor_info, data):
    ...
    var_atts = {
        ...
        'voltage': {'short_name' : 'voltage',
                    'long_name': sensor_info['voltage_description'],
                    'standard_name': 'none',
                    'units': 'volts',
                    'id_number' : sensor_info['voltage_id'],
                    'type' : sensor_info['voltage_type'],
                    'format' : sensor_info['voltage_format'],
                    'non_standard_units' : sensor_info['voltage_units'],
                    'range_min' : sensor_info['voltage_range_min'],
                    'range_max' : sensor_info['voltage_range_max'],
                   },
        'memory': {'short_name' : 'memory',
                   'long_name': sensor_info['memory_description'],
                   'standard_name': 'none',
                   'units' : 'none',
                   'id_number' : sensor_info['memory_id'],
                   'type' : sensor_info['memory_type'],
                   'format' : sensor_info['memory_format'],
                   'non_standard_units' : sensor_info['memory_units'],
                   'range_min' : sensor_info['memory_range_min'],
                   'range_max' : sensor_info['memory_range_max'],
                  },
        'interval': {'short_name' : 'interval',
                     'long_name': 'Averaging Interval',
                     'standard_name': 'averaging_interval',
                     'units': 'seconds/1000',
                     'id_number' : sensor_info['interval_id'],
                     'type' : sensor_info['interval_type'],
                     'format' : sensor_info['interval_format'],
                     'non_standard_units' : sensor_info['interval_units'],
                     'range_min' : sensor_info['interval_range_min'],
                     'range_max' : sensor_info['interval_range_max'],
                    },
        }
    ...
    var_inits = (
        ...
        ('voltage',  pycdf.NC.FLOAT, ('ntime',)),
        ('memory',   pycdf.NC.INT, ('ntime',)),
        ('interval', pycdf.NC.INT, ('ntime',)),
                )
