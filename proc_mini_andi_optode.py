...
Spongenet mini_andi optode parameters sponge data.
"""
...
def parser(platform_info, sensor_info, lines):
    ...
    data = {
        ...
        'o2concentration' : n.array(n.ones((num_samples,)) * n.nan, dtype=float),
        'airsaturation'   : n.array(n.ones((num_samples,)) * n.nan, dtype=float),
        'temperature'     : n.array(n.ones((num_samples,)) * n.nan, dtype=float),
        'calphase'        : n.array(n.ones((num_samples,)) * n.nan, dtype=float),
        'tcphase'         : n.array(n.ones((num_samples,)) * n.nan, dtype=float),
        'c1rph'           : n.array(n.ones((num_samples,)) * n.nan, dtype=float),
        'c2rph'           : n.array(n.ones((num_samples,)) * n.nan, dtype=float),
        'c1amp'           : n.array(n.ones((num_samples,)) * n.nan, dtype=float),
        'c2amp'           : n.array(n.ones((num_samples,)) * n.nan, dtype=float),
        'rawtemp'         : n.array(n.ones((num_samples,)) * n.nan, dtype=float),
           }
    ...
    for (sample_index, sample) in enumerate(_data.devices):
        ...
        try:
            data['o2concentration'][sample_index] = float(sample.sensors
                                                    [sensor_info["id_number"]].
                                                    points[sensor_info
                                                    ["o2concentration_description"]]
                                                    ["value"])
        except (KeyError, AttributeError, ):
            pass

        try:
            data['airsaturation'][sample_index] = float(sample.sensors
                                                  [sensor_info["id_number"]].
                                                  points[sensor_info
                                                  ["airsaturation_description"]]
                                                  ["value"])
        except (KeyError, AttributeError, ):
            pass
        ...
        try:
            data['calphase'][sample_index] = float(sample.sensors
                                                   [sensor_info["id_number"]].
                                                   points[sensor_info
                                                   ["calphase_description"]]
                                                   ["value"])
        except (KeyError, AttributeError, ):
            pass

        try:
            data['tcphase'][sample_index] = float(sample.sensors
                                                  [sensor_info["id_number"]].
                                                  points[sensor_info
                                                  ["tcphase_description"]]
                                                  ["value"])
        ...
        try:
            data['c1rph'][sample_index] = float(sample.sensors
                                                [sensor_info["id_number"]].
                                                points[sensor_info
                                                ["c1rph_description"]]
                                                ["value"])
        ...
        try:
            data['c2rph'][sample_index] = float(sample.sensors
                                                [sensor_info["id_number"]].
                                                points[sensor_info
                                                ["c2rph_description"]]
                                                ["value"])
        ...
        try:
            data['c1amp'][sample_index] = float(sample.sensors
                                                [sensor_info["id_number"]].
                                                points[sensor_info
                                                ["c1amp_description"]]
                                                ["value"])
        ...
        try:
            data['c2amp'][sample_index] = float(sample.sensors
                                                [sensor_info["id_number"]].
                                                points[sensor_info
                                                ["c2amp_description"]]
                                                ["value"])
        ...
        try:
            data['rawtemp'][sample_index] = float(sample.sensors
                                                  [sensor_info["id_number"]].
                                                  points[sensor_info
                                                  ["rawtemp_description"]]
                                                  ["value"])
...
def creator(platform_info, sensor_info, data):
    ...
    var_atts = {
        ...
        'o2concentration' : {'short_name' : 'o2concentration',
                     'long_name': sensor_info['o2concentration_description'],
                     'standard_name': 'oxygen_concentration',
                     'units': 'none',
                     'id_number' : sensor_info['o2concentration_id'],
                     'type' : sensor_info['o2concentration_type'],
                     'format' : sensor_info['o2concentration_format'],
                     'non_standard_units' : sensor_info['o2concentration_units'],
                     'range_min' : sensor_info['o2concentration_range_min'],
                     'range_max' : sensor_info['o2concentration_range_max'],
                   },
        'airsaturation' : {'short_name' : 'airsaturation',
                       'long_name': sensor_info['airsaturation_description'],
                       'standard_name': 'oxygen_saturation',
                       'units': 'none',
                       'id_number' : sensor_info['airsaturation_id'],
                       'type' : sensor_info['airsaturation_type'],
                       'format' : sensor_info['airsaturation_format'],
                       'non_standard_units' : sensor_info['airsaturation_units'],
                       'range_min' : sensor_info['airsaturation_range_min'],
                       'range_max' : sensor_info['airsaturation_range_max'],
                   },
        ...
        'calphase' : {'short_name' : 'calphase',
                      'long_name': sensor_info['calphase_description'],
                      'standard_name': 'none',
                      'units': 'none',
                      'id_number' : sensor_info['calphase_id'],
                      'type' : sensor_info['calphase_type'],
                      'format' : sensor_info['calphase_format'],
                      'non_standard_units' : sensor_info['calphase_units'],
                      'range_min' : sensor_info['calphase_range_min'],
                      'range_max' : sensor_info['calphase_range_max'],
                   },
        'tcphase' : {'short_name' : 'tcphase',
                     'long_name': sensor_info['tcphase_description'],
                     'standard_name': 'none',
                     'units': 'none',
                     'id_number' : sensor_info['tcphase_id'],
                     'type' : sensor_info['tcphase_type'],
                     'format' : sensor_info['tcphase_format'],
                     'non_standard_units' : sensor_info['tcphase_units'],
                     'range_min' : sensor_info['tcphase_range_min'],
                     'range_max' : sensor_info['tcphase_range_max'],
                   },
        'c1rph' : {'short_name' : 'c1rph',
                   'long_name': sensor_info['c1rph_description'],
                   'standard_name': 'none',
                   'units': 'none',
                   'id_number' : sensor_info['c1rph_id'],
                   'type' : sensor_info['c1rph_type'],
                   'format' : sensor_info['c1rph_format'],
                   'non_standard_units' : sensor_info['c1rph_units'],
                   'range_min' : sensor_info['c1rph_range_min'],
                   'range_max' : sensor_info['c1rph_range_max'],
                   },
        'c2rph' : {'short_name' : 'c2rph',
                   'long_name': sensor_info['c2rph_description'],
                   'standard_name': 'none',
                   'units': 'none',
                   'id_number' : sensor_info['c2rph_id'],
                   'type' : sensor_info['c2rph_type'],
                   'format' : sensor_info['c2rph_format'],
                   'non_standard_units' : sensor_info['c2rph_units'],
                   'range_min' : sensor_info['c2rph_range_min'],
                   'range_max' : sensor_info['c2rph_range_max'],
                   },
        'c1amp' : {'short_name' : 'c1amp',
                   'long_name': sensor_info['c1amp_description'],
                   'standard_name': 'none',
                   'units': 'none',
                   'id_number' : sensor_info['c1amp_id'],
                   'type' : sensor_info['c1amp_type'],
                   'format' : sensor_info['c1amp_format'],
                   'non_standard_units' : sensor_info['c1amp_units'],
                   'range_min' : sensor_info['c1amp_range_min'],
                   'range_max' : sensor_info['c1amp_range_max'],
                   },
        'c2amp' : {'short_name' : 'c2amp',
                   'long_name': sensor_info['c2amp_description'],
                   'standard_name': 'none',
                   'units': 'none',
                   'id_number' : sensor_info['c2amp_id'],
                   'type' : sensor_info['c2amp_type'],
                   'format' : sensor_info['c2amp_format'],
                   'non_standard_units' : sensor_info['c2amp_units'],
                   'range_min' : sensor_info['c2amp_range_min'],
                   'range_max' : sensor_info['c2amp_range_max'],
                   },
        'rawtemp' : {'short_name' : 'rawtemp',
                       'long_name': sensor_info['rawtemp_description'],
                       'standard_name': 'none',
                       'units': 'none',
                       'id_number' : sensor_info['rawtemp_id'],
                       'type' : sensor_info['rawtemp_type'],
                       'format' : sensor_info['rawtemp_format'],
                       'non_standard_units' : sensor_info['rawtemp_units'],
                       'range_min' : sensor_info['rawtemp_range_min'],
                       'range_max' : sensor_info['rawtemp_range_max'],
                   },
        }
    ...
    var_inits = (
        ...
        ('o2concentration', pycdf.NC.FLOAT, ('ntime',)),
    ...
    var_data = (
        ...
        ('o2concentration', data['o2concentration'][i]),
        ('airsaturation',   data['airsaturation'][i]),
        ('temperature',     data['temperature'][i]),
        ('calphase',        data['calphase'][i]),
        ('tcphase',         data['tcphase'][i]),
        ('c1rph',           data['c1rph'][i]),
        ('c2rph',           data['c2rph'][i]),
        ('c1amp',           data['c1amp'][i]),
        ('c2amp',           data['c2amp'][i]),
        ('rawtemp',         data['rawtemp'][i]),
        )
