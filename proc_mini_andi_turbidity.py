...
Spongenet mini_andi turbidity parameters sponge data.
"""
...
def parser(platform_info, sensor_info, lines):
    ...
    data = {
        ...
        'turbidity' : n.array(n.ones((num_samples,)) * n.nan, dtype=float),
           }
    ...
    for (sample_index, sample) in enumerate(_data.devices):
        ...
        try:
            data['turbidity'][sample_index] = float(sample.sensors
                                                    [sensor_info["id_number"]].
                                                    points[sensor_info
...
def creator(platform_info, sensor_info, data):
    ...
    var_atts = {
        ...
        'turbidity' : {'short_name' : 'turbidity',
                       'long_name': sensor_info['turbidity_description'],
                       'standard_name': 'turbidity',
                       'units': 'none',
                       'id_number' : sensor_info['turbidity_id'],
                       'type' : sensor_info['turbidity_type'],
                       'format' : sensor_info['turbidity_format'],
                       'non_standard_units' : sensor_info['turbidity_units'],
                       'range_min' : sensor_info['turbidity_range_min'],
                       'range_max' : sensor_info['turbidity_range_max'],
                   },
        }
    ...
    var_inits = (
        ...
        ('turbidity', pycdf.NC.FLOAT, ('ntime',)),
                )
